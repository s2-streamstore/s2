...
mod tests {
    ...
    use s2_api::v1::stream::{
        proto,
        s2s::{CompressionAlgorithm, SessionMessage, TerminalMessage},
    };
    ...
    use super::*;
    use crate::types::{
        AccountEndpoint, AppendRecord, AppendRecordBatch, BasinEndpoint, Compression, S2Endpoints,
    };
    ...
    impl TestServer {
            ...
            let handle = AbortOnDropHandle::new(tokio::spawn(async move {
                loop {
                    ...
                    tokio::spawn(async move {
                        let service =
                            service_fn(move |request| handle_request(request, captured.clone()));
        ...
        fn endpoints(&self) -> S2Endpoints {
            S2Endpoints::new(
                AccountEndpoint::new(&format!("http://{}", self.addr)).unwrap(),
                BasinEndpoint::new(&format!("http://{}", self.addr)).unwrap(),
            )
            .expect("matching test endpoints")
        }
    ...
    async fn handle_request(
        ...
        captured.lock().unwrap().push(CapturedRequest {
            ...
            encryption: request
                .headers()
                .get(&s2_common::encryption::S2_ENCRYPTION_HEADER)
                .and_then(|value| value.to_str().ok())
                .map(str::to_owned),
        });
        ...
        let response = match (
            ...
            (Method::POST, Some("application/protobuf")) => unary_append_response(),
            (Method::GET, Some("s2s/proto")) => streaming_read_response(),
            (Method::POST, Some("s2s/proto")) => streaming_terminal_response(
                StatusCode::UNPROCESSABLE_ENTITY,
                r#"{"code":"invalid","message":"streaming append rejected"}"#,
            ),
            (Method::GET, _) => unary_read_response(),
            _ => Response::builder()
    ...
    fn unary_append_response() -> Response<Full<Bytes>> {
        let ack = proto::AppendAck {
            start: Some(proto::StreamPosition {
                seq_num: 0,
                timestamp: 1,
            }),
        ...
        Response::builder()
            ...
            .body(Full::new(Bytes::from(ack.encode_to_vec())))
            .unwrap()
    }

    fn read_batch_proto() -> proto::ReadBatch {
        proto::ReadBatch {
            records: vec![proto::SequencedRecord {
                seq_num: 0,
                timestamp: 1,
                headers: vec![],
    ...
    fn unary_read_response() -> Response<Full<Bytes>> {
        let batch = read_batch_proto();
        Response::builder()
            .status(StatusCode::OK)
            .header(
                CONTENT_TYPE,
            ...
            .body(Full::new(Bytes::from(batch.encode_to_vec())))
            .unwrap()
    }

    fn streaming_read_response() -> Response<Full<Bytes>> {
        let batch = read_batch_proto();
        let frame = SessionMessage::regular(CompressionAlgorithm::None, &batch)
            .expect("encode read batch frame")
            .encode();
        Response::builder()
            .status(StatusCode::OK)
            .header(CONTENT_TYPE, HeaderValue::from_static("s2s/proto"))
            .body(Full::new(frame))
            .unwrap()
    }

    fn streaming_terminal_response(status: StatusCode, body: &str) -> Response<Full<Bytes>> {
        let frame = SessionMessage::Terminal(TerminalMessage {
    ...
    fn append_input(body: &str) -> AppendInput {
        AppendInput::new(
            AppendRecordBatch::try_from_iter([AppendRecord::new(body.to_owned()).unwrap()])
                .unwrap(),
        )
    }

    #[tokio::test]
    async fn with_encryption_propagates_to_stream_operations() {
        install_test_crypto_provider();
        let server = TestServer::spawn().await;
        let encryption = EncryptionConfig::aegis256([0x42; 32]);
        let config = S2Config::new("test-token")
            .with_endpoints(server.endpoints())
            .with_compression(Compression::None)
            .with_request_timeout(Duration::from_secs(1));
        let s2 = S2::new(config).expect("s2 client");
        ...
        let ack = stream.append(append_input("append")).await.expect("append");
        assert_eq!(ack.end.seq_num, 1);

        let batch = stream.read(ReadInput::new()).await.expect("read");
        assert_eq!(batch.records.len(), 1);
        assert_eq!(batch.records[0].body.as_ref(), b"hello");

        let mut batches = stream
            .read_session(ReadInput::new())
            .await
            .expect("read session");
        let batch = batches
            .next()
            ...
            .expect("streamed batch should succeed");
        assert_eq!(batch.records.len(), 1);
        assert_eq!(batch.records[0].body.as_ref(), b"hello");

        let session = stream.append_session(AppendSessionConfig::default());
        let ticket = session
            .submit(append_input("append-session"))
            .await
            .expect("append session ticket");
        let _ = ticket
            .await
            .expect_err("append session should surface terminal error");
        drop(session);

        let producer = stream.producer(ProducerConfig::default());
        let ticket = producer
            .submit(AppendRecord::new("producer").unwrap())
            .await
            .expect("producer ticket");
        let _ = ticket
            .await
            .expect_err("producer should surface terminal error");
        drop(producer);

        let expected_header = encryption
            .to_header_value()
            .to_str()
            .expect("encryption header value")
            .to_owned();
        let captured = server.captured();
        assert!(
            captured.len() >= 5,
            "expected at least one request per stream operation: {captured:?}"
        );
        for request in &captured {
            assert_eq!(
                request.path, "/v1/streams/test-stream/records",
                "unexpected request path"
            );
            assert_eq!(
                request.encryption.as_deref(),
                Some(expected_header.as_str()),
                "missing propagated encryption header for {request:?}"
            );
        }
        assert_eq!(
            captured
                ...
                .filter(|request| {
                    request.method == Method::POST
                        && request.content_type.as_deref() == Some("application/protobuf")
                })
                .count(),
            1,
            "expected one unary append request: {captured:?}"
        );
        assert_eq!(
            captured
                .iter()
                .filter(|request| {
                    request.method == Method::GET
                        && request.content_type.as_deref() != Some("s2s/proto")
                })
                .count(),
            1,
            "expected one unary read request: {captured:?}"
        );
        assert_eq!(
            captured
                .iter()
                ...
                .count(),
            1,
            "expected one streaming read request: {captured:?}"
        );
        assert!(
            captured
                .iter()
                ...
                .count()
                >= 2,
            "expected append_session and producer streaming requests: {captured:?}"
        );
    }
}
